...
export default function TopLosers({ datas }) {
  let positiveValues = [];
  let negativeValues = [];
  ...
  const sortedPositive = Array.from(positiveValues).sort((a, b) => {
    const changeA = String(a.change).split(" ")[0];
  ...
  const sortedNegative = Array.from(negativeValues).sort((a, b) => {
    ...
    return parseFloat(changeB) - parseFloat(changeA)
  })
  const sortedArray = [...sortedNegative, ...sortedPositive]
          ...
          >
            <div className="grid grid-cols-4 col-span-2">
              <div className="flex justify-center items-center">
                <Image className="rounded-full" src={`/images/${symbolString}.webp`} alt={`/images/${symbolString}.png`} width={20} height={20} />
              </div>
